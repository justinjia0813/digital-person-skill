...
- `config.yaml` / `build_manifest.json`：记录本次生成使用的 chat provider、embedding provider、skill version；其中 `build_manifest.json` 的 `vector.enabled` / `vector.skip_reason` 会显式标记是否跳过向量索引，并指向 `knowledge/vector_index/manifest.json`
- `knowledge/vector_index/manifest.json`：统一的向量索引清单，字段契约以 `src.models.VectorIndexManifest` 为唯一来源。collection metadata 使用 `COLLECTION_FIELD_NAMES`：`enabled`、`skip_reason`、`person_name`、`skill_name`、`skill_version`、`index_schema_version`、`chat_provider`、`chat_model`、`embedding_provider`、`embedding_model`、`collection_name`、`index_dir`、`source_fingerprint`、`built_at`；`manifest.json` 与版本目录下的 `metadata.json` 使用完整 `FIELD_NAMES`，即在前述字段基础上再补充 `total_documents`、`article_chunks`、`opinions`、`chunk_size`、`chunk_overlap`
...
- 如果使用 `--skip-vector`，仍会写入 `knowledge/vector_index/manifest.json`，其中 `enabled=false`、`skip_reason=explicit_skip_vector`；`collection_name` 和 `index_dir` 为 `null`，但仍保留本次生成的 provider / model / version / fingerprint 语义，便于后续补跑向量索引或排查
